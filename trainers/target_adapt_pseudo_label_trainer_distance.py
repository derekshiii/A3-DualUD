...
from utils import shapecompactnessless, ShapeCompactnessLoss
import numpy as np
...
class PseudoLabel_Trainer3():
    ...
    def initialize(self):
        ...
        if self.opt['patient_level_dataloader']:
            train_dataset = PatientDataset(self.opt['data_root'], self.opt['target_sites'], phase='train',
                                           split_train=True, weak_strong_aug=True)
            patient_sampler = MyBatchSampler(train_dataset, self.opt['batch_size'])
            self.train_dataloader = DataLoader(train_dataset, batch_sampler=patient_sampler,
        ...
        ## losses
        self.criterion_pseudo = nn.CrossEntropyLoss(weight=torch.tensor([0.1, 1, 1, 1, 1]).to(self.opt['gpu_id']),
                                                    reduction='none')
    ...
    def save_models(self, step, dice):
        if step != 0:
            ...
            torch.save(state,
                       os.path.join(checkpoint_dir, 'saved_models', 'model_step_{}_dice_{:.4f}.pth'.format(step, dice)))
    ...
    def save_best_models(self, step, dice):
        ...
        torch.save(state, os.path.join(checkpoint_dir, 'saved_models',
                                       'best_model_step_{}_dice_{:.4f}.pth'.format(step, dice)))
    ...
    def train_one_step(self, data):
        ...
        # emamodel是presourcemode需要去噪，model是目标模型
        # get losses
        self.model.train()
        ...
        with torch.no_grad():
            if self.use_ema:
                ...
                pseudo_label[pseudo_label > 0.7] = 1.0; pseudo_label[pseudo_label <= 0.7] = 0.0
                ...
                mask = torch.cat((mask_0, mask_1, mask_2, mask_3, mask_4), dim=1)
                #
                ...
                #                              keepdim=True)
                # target_2_obj_cnt = torch.sum(mask_2_obj * target_2_obj * prediction_small[:, 2:3, ...], dim=[0, 2, 3],
                #                              keepdim=True)
                ...
                proto_pseudo = F.interpolate(mask, size=data[1].size()[2:], mode='nearest')
                prob_w = prediction_yuan * proto_pseudo
                # # ============================================================
        ...
        lambda_val = 0.5  # 调整紧凑度损失的权重
        loss_function = ShapeCompactnessLoss(lambda_val)
        loss_seg_pixel = loss_function(prob_s, pseudo_label)


        pseudo_ce_loss = pseudo_ce_loss * mask
        loss_seg_pixel = loss_seg_pixel * mask

        loss = (pseudo_ce_loss+loss_seg_pixel).mean()
    ...
    def train(self):
        for epoch in range(self.start_epoch, self.total_epochs):
            ...
            for it, (img_s, img_w, segs, _) in enumerate(train_iterator):
                ...
                with self.iter_counter.time_measurement("train"):
                    ...
                    train_iterator.set_postfix(ce_loss=train_losses['pseudo_ce_loss'].item() / (it + 1),
                                               quality=trade_off['quality'].item(),
                                               quantity=trade_off['quantity'].item())

                with self.iter_counter.time_measurement("maintenance"):
                    if self.iter_counter.needs_displaying():
                        visuals = {'images': img_s[:, 1].detach().cpu().numpy(),
                    ...
                    self.visualizer.plot_current_metrics(self.iter_counter.steps_so_far, trade_off,
                                                         'Quantity_vs_Quality')
                    if self.iter_counter.needs_evaluation_steps():
                        ...
                        self.visualizer.plot_current_metrics(self.iter_counter.steps_so_far, val_metrics['dice'],
                                                             'Dice_metrics')
                        self.schedular.step()
                self.iter_counter.record_one_iteration()
            self.iter_counter.record_one_epoch()
        # best_model_path = '/media/dell/disk2/zhoujie/MICCAI23-deeplab_TCT/target_adapt/DeepLab_Abdomen_MR2CT_Adapt_FixMatch_fold5/exp_0_time_2024-08-12 15-11-00/saved_models/best_model_step_200_dice_0.8075.pth'
        ...
        #         predict = self.validate_one_step([val_imgs, val_segs])
        #
        #         # output = torch.softmax(predict, dim=1)
        ...
        #
        #
        #         results = "prediction_Abdomen_fold5"
        #         if not os.path.exists(results):
        #             os.makedirs(results)
        #         results_gt = "prediction_gt_fold5"
