...
from losses import Entropy_KLProp_Loss
...
class PFA_Trainer():
    ...
    def initialize(self):
        ...
        self.source_prototypes = self.model.classifier.final_layer.weight.view((self.opt['num_classes'] * self.opt['num_prototypes'], self.opt['output_dim']))
        self.model.classifier.final_layer.requires_grad = False
        ...
        # self.criterion_proto = Entropy_KLProp_Loss(self.opt['nav_t'], self.opt['beta'], self.opt['num_classes'],
        #                                  self.opt['gpu_id'], self.opt['s_par'])
    ...
    def train_one_step(self, data):
        ...
        target_f,predict = self.model(imgs,only_feature=False)
    ...
    def train(self):
            ...
            for it, (images,segs,_) in enumerate(train_iterator):
                ...
                with self.iter_counter.time_measurement("maintenance"):
                    ...
                    if self.iter_counter.needs_evaluation_steps():
                        ...
                        self.schedular.step(val_metrics['dice']['dice_avg'])
                        # self.schedular.step()
                self.iter_counter.record_one_iteration()
            self.iter_counter.record_one_epoch()
        # best_model_path = '/media/dell/disk2/zhoujie/MICCAI23-deeplab_TCT/source_seg/best_model_epoch_21_dice_0.9350.pth'
        ...
        #
        #
        #         results = "/media/dell/disk2/zhoujie/MICCAI23-deeplab_TCT/pvalue/supervised1"
