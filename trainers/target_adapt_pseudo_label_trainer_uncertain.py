...
class PseudoLabel_Trainer2():
    ...
    def initialize(self):
        ...
        # self.ema_model = self.model.to(self.opt['gpu_id'])
        ...
        self.criterion_pseudo = nn.CrossEntropyLoss(weight=torch.tensor([0.1, 1, 1, 1, 1]).to(self.opt['gpu_id']),
                                                    reduction='none')
        # self.criterion_pseudo = nn.CrossEntropyLoss(weight=torch.tensor([0.1, 1, 2,  1]).to(self.opt['gpu_id']),
        #                                             reduction='none')
    ...
    def train_one_step(self, data):
        ...
        for i in range(10):
            with torch.no_grad():
                features[i, ...], preds[i, ...]= self.ema_model(imgs_w)
        preds1 = torch.softmax(preds, dim=2)
        preds = torch.softmax(preds / 2.0, dim=2)
        std_map = torch.std(preds, dim=0)

        std_map_small = F.interpolate(std_map, size=(256,256), mode='bilinear', align_corners=True)
        mask = torch.zeros_like(std_map_small)
        mask[std_map_small < 0.005] = 1.0
        ...
        prob_w= rearrange(pseudo_label, 'b c h w -> (b h w) c')


        pseudo_label_1 = torch.argmax(prob_w, dim=1)
        ...
        pseudo_ce_loss = self.criterion_pseudo(prob_s, pseudo_label_1)
        ...
        tp_region = (pseudo_label_1[fg_idxs] == gt[fg_idxs]) * (mask[fg_idxs] / torch.sum(mask[fg_idxs]))
        ...
        pseudo_label = rearrange(pseudo_label_1, '(b h w) -> b h w', b=b, h=h, w=w)
            ...
            for it, (img_s, img_w, segs, _) in enumerate(train_iterator):
                ...
                with self.iter_counter.time_measurement("maintenance"):
                    ...
                    if self.iter_counter.needs_evaluation_steps():
                        ...
                        # pdb.set_trace()
                        val_metrics['dice'], val_metrics_std['dice_std'] = mean_dice(pred_results_list, gt_segs_list, self.opt['num_classes'],
                                                        self.opt['organ_list'])
                        print(val_metrics)
