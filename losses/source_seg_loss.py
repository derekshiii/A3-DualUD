...
class BoundarySensitiveDiceLoss(nn.Module):
    def __init__(self, config=None, boundary_weight=1.0):
        super(BoundarySensitiveDiceLoss, self).__init__()
    ...
    def forward(self, preds, target, weights=None):
        ...
        for i in range(self.num_classes):
            ...
            # 计算边界损失
            boundary_loss = self.calculate_boundary_loss(seg[:, i], target[:, i])

            # 计算 Dice 损失
            diceLoss = self.dice_criterion(seg[:, i], target[:, i])

            # 结合 Dice 损失和边界损失
            loss = diceLoss + self.boundary_weight * boundary_loss
    ...
    def calculate_boundary_loss(self, pred, target):
        """
        计算边界损失。
        使用 Sobel 算子计算图像的梯度，并计算预测结果和真实标签的梯度差异。
        """
        # 使用 Sobel 算子计算梯度
        ...
        # 计算边界损失
        boundary_loss = torch.mean(grad_diff_x + grad_diff_y)
