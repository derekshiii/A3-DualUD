...
def cal_average_surface_distance(input,target):
    ...
    surface_distances = surfdist.compute_surface_distances(input, target, spacing_mm=(0.75,0.75, 1))
...
def assd_numpy(binary_segmentation, binary_gt_label):
    binary_segmentation = binary_segmentation.cpu().numpy().astype(np.bool8)
    binary_gt_label = binary_gt_label.cpu().numpy().astype(np.bool8)
    ...
    if np.sum(binary_segmentation) > 0 and np.sum(binary_gt_label) > 0:
        ...
        return metric.binary.assd(binary_segmentation, binary_gt_label,voxelspacing)
...
def MultiASD(preds,target,num_classes,include_bg=False):
    ...
    # new_target = torch.argmax(target, dim=3)
    if isinstance(preds, dict):
        seg = preds['seg']
    else:
        seg = preds
...
def mean_dice(results, gt_seg_maps,num_classes,organ_list):
    ...
    for j,organ in enumerate(organ_list):
        ...
        dice_metric_std['{:}_dice'.format(organ)] = total_dice_mat[:, j].std()
    dice_metric['dice_avg'] = total_dice_mat.mean()
    return dice_metric,dice_metric_std

def mean_assd(results, gt_seg_maps,num_classes,organ_list):
    num_imgs = len(results)
    assert len(gt_seg_maps) == num_imgs
    total_assd_mat = []
    assd_metric = {}
    assd_metric_std = {}
    for i in range(num_imgs):
        assd_coef = MultiASD(results[i],gt_seg_maps[i],num_classes)
        total_assd_mat.append(assd_coef)
    total_assd_mat = np.array(total_assd_mat)
    for j,organ in enumerate(organ_list):
        assd_metric['{:}_assd'.format(organ)] = total_assd_mat[:,j].mean()
        assd_metric_std['{:}_assd'.format(organ)] = total_assd_mat[:, j].std()
    assd_metric['avg_assd'] = total_assd_mat.mean()

    return assd_metric,assd_metric_std
